...
# parse the input training file(.tsv)
def collect_vocabs(train_path, with_POS=False, with_NER=False, with_DA=True):
    all_labels = set()
    all_words = set()
    all_labels_d = None
    all_POSs = None
    ...
    if with_DA:
        for line in infile:
            ...
            label_d = items[0]
            label = items[1]
            sentence1 = re.split("\\s+", items[2].lower())
            sentence2 = re.split("\\s+", items[3].lower())
            all_labels_d.add(label_d)
        ...
        for word in all_words:
            for char in word:
                all_chars.add(char)
        return (all_words, all_chars, all_labels, all_labels_d, all_POSs, all_NERs)
...
def evaluate(dataStream, valid_graph, sess, outpath=None, label_vocab=None, mode='prediction', char_vocab=None,
    ...
    for batch_index in xrange(dataStream.get_num_batch()):
        ...
        (label_batch, sent1_batch, sent2_batch, label_id_batch,label_d_id_batch, word_idx_1_batch, word_idx_2_batch,
        ...
        if outpath is not None:
            if mode == 'prediction':
                ...
                for i in xrange(len(label_batch)):
                    outline = label_batch[i] + "\t" + label_vocab.getWord(predictions[i]) + "\t" + sent1_batch[
                        i] + "\t" + sent2_batch[i] + "\n"
...
def main(_):
    ...
    ###These ops are for there existed a trained model and data
    word_vocab = Vocab(word_vec_path, fileformat='txt3')
    best_path = path_prefix + '.best.model'
    char_path = path_prefix + ".char_vocab"
    label_path = path_prefix + ".label_vocab"
    label_d_path = path_prefix + ".label_d_vocab"
    ...
    if os.path.exists(best_path):
        ...
        if FLAGS.with_DA: label_d_vocab = Vocab(label_d_path, fileformat='txt2')
    ...
    ###
        ...
        (all_words, all_chars, all_labels,all_labels_d, all_POSs, all_NERs) = collect_vocabs(train_path, with_POS=FLAGS.with_POS,
                                                                                with_NER=FLAGS.with_NER,with_DA=FLAGS.with_DA)
        ...
        if FLAGS.with_DA:
            print('Number of domain labels: {}'.format(len(all_labels_d)))
            label_d_vocab=Vocab(fileformat='voc', voc=all_chars, dim=2)
            label_d_vocab.dump_to_txt2(label_d_path)
    ...
    if FLAGS.with_DA:
        dom_classes = label_d_vocab.size()
    print('Build SentenceMatchDataStream ... ')
    trainDataStream = SentenceMatchDataStream(train_path, word_vocab=word_vocab, char_vocab=char_vocab,
                                              POS_vocab=POS_vocab, NER_vocab=NER_vocab, label_vocab=label_vocab,label_d_vocab = label_d_vocab,
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope("Model", reuse=None, initializer=initializer):
            train_graph = SentenceMatchModelGraph(num_classes, dom_classes=dom_classes, word_vocab=word_vocab, char_vocab=char_vocab,
                                                  ...
                                                  aggregation_lstm_dim=FLAGS.aggregation_lstm_dim, is_training=True,
                                                  MP_dim=FLAGS.MP_dim,
        ...
        with tf.variable_scope("Model", reuse=True, initializer=initializer):
            valid_graph = SentenceMatchModelGraph(num_classes=num_classes,dom_classes=dom_classes, word_vocab=word_vocab, char_vocab=char_vocab,
                                                  POS_vocab=POS_vocab, NER_vocab=NER_vocab,
                                                  dropout_rate=FLAGS.dropout_rate, learning_rate=FLAGS.learning_rate,
                                                  optimize_type=FLAGS.optimize_type,
                                                  lambda_l2=FLAGS.lambda_l2, char_lstm_dim=FLAGS.char_lstm_dim,
        ...
        ### Just to save the model parameters for further reuse
        vars_ = {}
        ...
        for step in xrange(max_steps):
            ...
            if (step + 1) % trainDataStream.get_num_batch() == 0 or (step + 1) == max_steps:
                ...
                accuracy = evaluate(devDataStream, valid_graph, sess, char_vocab=char_vocab, POS_vocab=POS_vocab,
                                    NER_vocab=NER_vocab)
    ...
    with tf.Graph().as_default():
        ...
        with tf.variable_scope("Model", reuse=False, initializer=initializer):
            valid_graph = SentenceMatchModelGraph(num_classes, dom_classes, word_vocab=word_vocab, char_vocab=char_vocab,
                                                  POS_vocab=POS_vocab, NER_vocab=NER_vocab,
                                                  dropout_rate=FLAGS.dropout_rate, learning_rate=FLAGS.learning_rate,
                                                  optimize_type=FLAGS.optimize_type,
                                                  lambda_l2=FLAGS.lambda_l2, char_lstm_dim=FLAGS.char_lstm_dim,
        ...
        accuracy = evaluate(testDataStream, valid_graph, sess, char_vocab=char_vocab, POS_vocab=POS_vocab,
                            NER_vocab=NER_vocab)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fix_word_vec', default=False, help='Fix pre-trained word embeddings during training.',
                        action='store_true')
...
#######
    parser.add_argument('--with_DA', default=True, help='Utilize domain adaptation.', action='store_true')

    parser.add_argument('--word_level_MP_dim', type=int, default=-1,
                        help='Number of perspectives for word-level matching.')
    parser.add_argument('--with_match_highway', default=False, help='Utilize highway layers for matching layer.',
                        action='store_true')
